...
namespace
{
	...
	bool ConvertScalarJsonValueToFPropertyWithContainer(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags)
	{
		...
		else if (FNumericProperty *NumericProperty = CastField<FNumericProperty>(Property))
		{
			...
			else if (NumericProperty->IsInteger())
			{
				if (JsonValue->Type == EJson::String)
				{
					// parse string -> int64 ourselves so we don't lose any preciSCn going through AsNumber (aka double)
	...
	bool JsonValueToFPropertyWithContainer(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags)
	{
		...
		if (!bJsonArray)
		{
			...
			{
				//Begin custom workaround - support string -> binary array converSCn
...
FString USCJsonConvert::ToJsonString(const TSharedPtr<FJsonObject>& JsonObject)
{
...
FString USCJsonConvert::ToJsonString(const TArray<TSharedPtr<FJsonValue>>& JsonValueArray)
{
	FString OutputString;
	TSharedRef< FCondensedJsonStringWriter > Writer = FCondensedJsonStringWriterFactory::Create(&OutputString);
...
FString USCJsonConvert::ToJsonString(const TSharedPtr<FJsonValue>& JsonValue)
{
...
USCJsonValue* USCJsonConvert::ToSCJsonValue(const TArray<TSharedPtr<FJsonValue>>& JsonValueArray)
{
	...
	USCJsonValue* ResultValue = NewObject<USCJsonValue>();
...
TSharedPtr<FJsonValue> USCJsonConvert::JsonStringToJsonValue(const FString& JsonString)
{
	//Null
...
TSharedPtr<FJsonValue> USCJsonConvert::ToJsonValue(const TSharedPtr<FJsonObject>& JsonObject)
{
	return MakeShareable(new FJsonValueObject(JsonObject));
}

TSharedPtr<FJsonValue> USCJsonConvert::ToJsonValue(const FString& StringValue)
{
	return MakeShareable(new FJsonValueString(StringValue));
}

TSharedPtr<FJsonValue> USCJsonConvert::ToJsonValue(double NumberValue)
{
	return MakeShareable(new FJsonValueNumber(NumberValue));
}

TSharedPtr<FJsonValue> USCJsonConvert::ToJsonValue(bool BoolValue)
{
	return MakeShareable(new FJsonValueBoolean(BoolValue));
}

TSharedPtr<FJsonValue> USCJsonConvert::ToJsonValue(const TArray<uint8>& BinaryValue)
{
	return MakeShareable(new FJsonValueBinary(BinaryValue));
}

TSharedPtr<FJsonValue> USCJsonConvert::ToJsonValue(const TArray<TSharedPtr<FJsonValue>>& ArrayValue)
{
...
TArray<TSharedPtr<FJsonValue>> USCJsonConvert::JsonStringToJsonArray(const FString& JsonString)
{
	TArray < TSharedPtr<FJsonValue>> RawJsonValueArray;
	TSharedRef< TJsonReader<> > Reader = TJsonReaderFactory<>::Create(*JsonString);
...
TSharedPtr<FJsonObject> USCJsonConvert::ToJsonObject(const FString& JsonString)
{
	TSharedPtr< FJsonObject > JsonObject = MakeShareable(new FJsonObject);
	TSharedRef< TJsonReader<> > Reader = TJsonReaderFactory<>::Create(*JsonString);
...
TSharedPtr<FJsonObject> USCJsonConvert::ToJsonObject(UStruct* StructDefinition, void* StructPtr, bool IsBlueprintStruct, bool BinaryStructCppSupport /*= false */)
{
			...
			EnumOverrideExportCallback.BindLambda([](FProperty* Property, const void* Value)
			{
				...
				else if (FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Property))
				{
					...
					if (ArrayProperty->Inner->IsA<FByteProperty>())
					{
						...
						return USCJsonConvert::ToJsonValue(ByteArray);
					}
				}
...
TSharedPtr<FJsonObject> USCJsonConvert::MakeJsonObject()
{
	return MakeShareable(new FJsonObject);
}

bool USCJsonConvert::JsonObjectToUStruct(TSharedPtr<FJsonObject> JsonObject, UStruct* Struct, void* StructPtr, bool IsBlueprintStruct /*= false*/, bool BinaryStructCppSupport /*= false*/)
{
	if (IsBlueprintStruct || BinaryStructCppSupport)
	{
		//Json object we pass will have their trimmed BP names, e.g. boolKey vs boolKey_8_EDBB36654CF43866C376DE921373AF23
		//so we have to match them to the verbose verSCns, get a map of the names
...
bool USCJsonConvert::JsonFileToUStruct(const FString& FilePath, UStruct* Struct, void* StructPtr, bool IsBlueprintStruct /*= false*/)
{
...
bool USCJsonConvert::ToJsonFile(const FString& FilePath, UStruct* Struct, void* StructPtr, bool IsBlueprintStruct /*= false*/)
{
...
void USCJsonConvert::TrimValueKeyNames(const TSharedPtr<FJsonValue>& JsonValue)
{
	//Array?
	if (JsonValue->Type == EJson::Array)
	{
...
bool USCJsonConvert::TrimKey(const FString& InLongKey, FString& OutTrimmedKey)
{
...
void USCJsonConvert::SetTrimmedKeyMapForStruct(TSharedPtr<FTrimmedKeyMap>& InMap, UStruct* Struct)
{
...
void USCJsonConvert::SetTrimmedKeyMapForProp(TSharedPtr<FTrimmedKeyMap>& InMap, FProperty* InnerProperty)
{
...
void USCJsonConvert::ReplaceJsonValueNamesWithMap(TSharedPtr<FJsonValue>& JsonValue, TSharedPtr<FTrimmedKeyMap> KeyMap)
{
	if (JsonValue->Type == EJson::Object)
	{
		...
		FString PreviewPreValue = USCJsonConvert::ToJsonString(Object);
		...
		FString PreviewPostValue = USCJsonConvert::ToJsonString(Object);
...
FString USCJsonConvert::EnumToString(const FString& enumName, const int32 value)
{
	UEnum* pEnum = FindObject<UEnum>(ANY_PACKAGE, *enumName);
	return *(pEnum ? pEnum->GetNameStringByIndex(static_cast<uint8>(value)) : "null");
}

TSharedPtr<FJsonObject> USCJsonConvert::Merge(const TSharedPtr<FJsonObject>& Target, const TSharedPtr<FJsonObject>& Sources)
{
	TSharedPtr<FJsonObject> TargetObject = Target;
	auto SourceValues = Sources->Values;
