...
TArray<uint8> FJsonValueBinary::AsBinary(const TSharedPtr<FJsonValue>& InJsonValue)
{
	...
	else if (InJsonValue->Type == EJson::String)
	{
		...
		{
			UE_LOG(SCJson, Warning, TEXT("FJsonValueBinary::AsBinary couldn't decode %s as a binary."), *InJsonValue->AsString());
		}
		return DecodedArray;
	}
	else
...
USCJsonValue* USCJsonValue::ConstructJsonValue(UObject* WorldContextObject, const TSharedPtr<FJsonValue>& InValue)
{
...
TArray<uint8> USCJsonValue::AsBinary()
{
	...
	if (JsonVal->Type == EJson::String)
	{
		...
		else
		{
			...
			//Empty our array if converSCn failed
			if (!DidConvert)
			{
				ByteArray.Empty();
			}
...
{
	UE_LOG(SCJson, Error, TEXT("Json Value of type '%s' used as a '%s'."), *GetTypeString(), *InType);
}
